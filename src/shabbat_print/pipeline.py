...
def build(
    ...
    for index, document in enumerate(documents):
        # Each document gets its own subdirectory rather than sharing out_dir
        # directly: render()'s output filename is derived from the message's
        # identifier, and bulk mail without a Message-ID header falls back to
        # the sender address in extract.py, so two such messages from the
        # same sender would otherwise collide and silently overwrite one
